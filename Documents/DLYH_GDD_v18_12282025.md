...
| Message popup position | Bottom of screen by default; remembers player's dragged position |
| Main menu trivia | Rotating historical facts add thematic flavor |
...
### Phase 5: UX Polish and Multiplayer - TODO

| Item | Status |
|------|--------|
| Setup screen UX redesign (wizard flow) | TODO |
...
**Problem:** Current setup feels cluttered - all 15+ interactive elements visible simultaneously (name, 8 color buttons, 3 dropdowns, 2 random buttons, 3-4 word rows with compass buttons, full grid, full letter tracker, Start button).

**Solution:** Progressive disclosure wizard flow inspired by Dots and Boxes setup screen. Break the all-at-once setup into focused steps.

**Design Principles (from DNB analysis):**
- Maximum 2-3 choices visible at any time
- Card-based selection for major choices
- Only show elements when they become relevant
...
- Name input + 8 color swatches
- Clean, friendly, uncluttered
- Continue when both selected

---

**Step 2: Difficulty Choice (Card-Based)**
...
**Preset Cards:**
| Preset | Grid | Words | Difficulty |
|--------|------|-------|------------|
| Quick | 6x6 | 3 | Easy |
| Classic | 8x8 | 4 | Normal |
| Expert | 10x10 | 3 | Hard |
| Custom | (expands) | (expands) | (expands) |

- Clicking a preset card advances to Step 3
- "Custom" expands inline to show Grid/Words/Difficulty dropdowns
- Most players never need custom (presets cover 80% of use cases)
...
|         ENTER YOUR WORDS         |
|                                  |
|  Word 1 (3 letters): C A T ✓    |
|  Word 2 (4 letters): _ _ _ _    |
|  Word 3 (5 letters): _ _ _ _ _  |
|                                  |
|  [autocomplete suggestions...]   |
|                                  |
|  [PICK RANDOM]    [CONTINUE →]  |
+----------------------------------+
```

- Grid NOT visible - just focused word entry
- Clean autocomplete dropdown
- "Pick Random" fills empty slots
- Continue enabled when all words entered
- Letter tracker can appear here for keyboard input

---

**Step 4: Word Placement**

```
+----------------------------------+
|       PLACE YOUR WORDS           |
|                                  |
|  Tap a word, then tap the grid  |
|  to place it.                    |
|                                  |
|     A B C D E F                  |
|   1 [ ][ ][ ][ ][ ][ ]           |
...
|  [CAT ✓] [RACK] [SURGE]         |
|                                  |
|  [RANDOM PLACE]  [START GAME →] |
+----------------------------------+
```

- Grid appears only now when needed
- Words displayed as tappable chips
- Selected word highlights, waiting for grid tap
- Placed words show checkmark
- "Random Place" auto-places unplaced words
- Start Game enabled when all words placed

---

**Implementation Approach (uGUI):**

1. **Create 4 child panels** under SetupScreen:
   - `PersonalSetupPanel`
   - `DifficultyPanel`
   - `WordEntryPanel`
   - `PlacementPanel`

2. **Use SetActive() for visibility** (like DNB's `.hidden` class):
```csharp
public enum SetupStep { Personal, Difficulty, Words, Placement }
private SetupStep _currentStep;

void ShowStep(SetupStep step) {
    _personalPanel.SetActive(step == SetupStep.Personal);
    _difficultyPanel.SetActive(step == SetupStep.Difficulty);
    _wordsPanel.SetActive(step == SetupStep.Words);
    _placementPanel.SetActive(step == SetupStep.Placement);
    _currentStep = step;
}
```

3. **Reuse existing components:**
   - `SetupSettingsPanel` logic stays, reorganized into step panels
   - Word rows move to WordEntryPanel
   - Grid moves to PlacementPanel
   - Color picker stays in PersonalSetupPanel

4. **Add SetupWizardController:**
   - Manages step progression
   - Validates each step before allowing Continue
   - Handles Back button to return to previous step

**Benefits:**
- Reduces cognitive load (2-4 elements per screen vs 15+)
- Clearer mental model (linear path through setup)
- More welcoming for new players
- Grid/placement complexity hidden until relevant
- Presets make common configurations one-click
