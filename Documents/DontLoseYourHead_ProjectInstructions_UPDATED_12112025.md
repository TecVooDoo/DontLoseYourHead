...
**Examples:**
- `ProjectInstructions_UPDATED_12112025.md`
- `GDD_UPDATED_12112025.md`
...
2. Win/Lose condition UI feedback
...
| GameplayUIController.cs | Two-panel gameplay system | ~1,600 | Complete Dec 11, 2025 |
| SetupModeController.cs | Keyboard input routing | ~150 | |
| WordPatternRow.cs | Individual word entry rows | ~800 | Added word guess mode Dec 11 |
...
### Word Guess Mode Architecture (Dec 11, 2025)

**Flow:**
1. Player clicks "Guess Word" button on opponent's word row
2. All "Guess Word" buttons hide
3. Row shows Backspace/Accept/Cancel buttons
4. Letter tracker converts to keyboard mode (all white)
5. Player types letters (discovered letters stay fixed)
6. Accept validates and processes guess
7. Cancel exits without penalty

**Key Fields:**
- `_letterTrackerInKeyboardMode` - Tracks keyboard mode state
- `_activeWordGuessRow` - Currently active row for guessing
- `_wordSolved` flag in WordPatternRow - Prevents button re-show

### Three-Color Grid Cell System (Dec 11, 2025)

| Color | Meaning | When Applied |
|-------|---------|--------------|
| Green | Hit - letter known | Coordinate guess + letter already guessed |
| Yellow | Hit - letter unknown | Coordinate guess but letter not yet guessed |
| Red | Miss - empty cell | Coordinate guess on empty cell |
...
### New Input System Usage (Dec 11, 2025)

**CRITICAL:** Use Unity's New Input System, NOT legacy Input class.
...
### Permanent State Pattern (Dec 11, 2025)
...
**Key insight:** Simple Hide() calls can be overridden by subsequent Show() calls. Use boolean flags for permanent state changes.
