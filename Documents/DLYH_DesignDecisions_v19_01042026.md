...
| 16.0 | Dec 22, 2025 | Drug word filtering, board reset fixes, gameplay settings button, version display |
| 17.0 | Dec 25, 2025 | Document naming convention standardization |
| 18.0 | Dec 28, 2025 | Version sync across all core docs |
| 19.0 | Jan 4, 2026 | New UI system - LetterCellUI and WordPatternRowUI components |
...
### Final Execution Audio Sequence
...
### Miss Limit Difficulty Modifiers
...
**Decision:** Filter drug-related words from the word bank to prevent inappropriate content.

**Banned Words:**
heroin, cocaine, meth, crack, weed, opium, morphine, ecstasy, molly, dope, smack, coke

**Implementation:**
- HashSet in `WordBankImporter.cs` with `StringComparer.OrdinalIgnoreCase`
- `FilterWordsByLength()` checks each word against banned list
- Words removed at import time, not runtime

**Rationale:** Playtest feedback reported "heroin" appearing in word list. Drug references are inappropriate for the game's intended audience. HashSet provides O(1) lookup with case-insensitive matching.

### Settings Button During Gameplay

**Decision:** Allow players to access Settings without returning to main menu.

**Implementation:**
- `GameplaySettingsButton.cs` - Simple button component
- `SettingsPanel.ShowFromGameplay()` - Opens settings with gameplay flag
- Contextual back behavior returns to correct screen

**Behavior:**
- From gameplay: Opens Settings, hides main menu elements, Back returns to gameplay
- From main menu: Standard Settings behavior, Back returns to main menu

**Rationale:** Players shouldn't have to abandon a game to adjust volume or other settings.

### Setup Screen UX Redesign (Wizard Flow)

**Decision:** Redesign the setup screen from an all-at-once layout to a progressive disclosure wizard flow.

**Problem Analysis:**
The current setup screen shows 15+ interactive elements simultaneously:
- Player name input
- 8 color picker buttons
- 3 dropdown menus (grid size, word count, difficulty)
- 2 random action buttons
- 3-4 word entry rows with compass buttons
- Full grid display
- Full letter tracker keyboard
- Start button

This creates cognitive overload and makes the setup feel cluttered compared to modern game UIs.

**Inspiration: Dots and Boxes Setup**
Analysis of the DNB HTML5 game setup screen revealed key UX patterns:
- **Progressive Disclosure:** Each step reveals only the next 2-3 options
- **Card-Based Selection:** Major choices use clickable cards, not dropdowns
- **Conditional Visibility:** Sections completely hidden until relevant
- **Linear Flow:** Clear path through setup with Continue buttons
- **Generous Spacing:** Each step can breathe without visual competition

**Proposed Flow:**
1. **Personal Setup** - Name + color only
2. **Difficulty Choice** - Card presets (Quick/Classic/Expert) + Custom option
3. **Word Entry** - Focused word input, no grid visible
4. **Word Placement** - Grid appears only when placing words

**Key Design Principles:**
- Maximum 2-4 interactive elements per step
- Presets eliminate 80% of dropdown usage
- Grid hidden until Step 4 (reduces visual noise)
- Each step validates before allowing Continue

**Implementation Approach:**
- Stays entirely in uGUI (no UI Toolkit migration needed)
- Use `SetActive()` to show/hide step panels
- Reuse existing components, reorganized into step panels
- Add `SetupWizardController` to manage step progression

**Rationale:**
The DNB setup "feels better" not because of HTML5 vs Unity, but because of UX design. The same progressive disclosure pattern works in uGUI. This is a UX restructure, not a technology change.

---

### Board Reset on New Game

**Decision:** Fully reset game state when starting a new game after completing one.

**Components Reset:**
1. **GameplayUIController.ResetGameplayState():**
   - Guessed word lists (both players)
   - Letter tracker buttons (both panels)
   - Guillotines (blade, head, face)
   - Pending game result

2. **SetupSettingsPanel.ResetForNewGame():**
   - Player grid (placed words, word pattern rows)
   - Grid initialization
   - Player name input
   - Button states

3. **GuillotineDisplay.Reset():**
   - Blade position (stored on Initialize)
   - Head position (stored on Initialize)
   - Face expression (via HeadFaceController.ResetFace())

**Rationale:** Playtest feedback showed old game data persisting into new games. Complete state reset ensures clean slate.

### Version Display

**Decision:** Display application version on main menu.

**Implementation:**
- `VersionDisplay.cs` - Component for TextMeshProUGUI
- Uses `Application.version` from Project Settings
- Configurable format string (default: "v{0}")

**Rationale:** Players and testers need to verify which version they're running. Makes bug reports more useful.
...
**21. Store Original Positions for Reset**
When UI elements animate or move, store their original position on Initialize for proper reset:
```csharp
...
**22. Kill DOTween Before Reset**
Always kill active DOTween animations before resetting position to avoid conflicts:
```csharp
...
**23. Scene File Protection**
Unity scene files contain layout data that can be accidentally modified. If unexpected layout issues appear:
1. Check scene file changes: `git diff Assets/DLYH/Scenes/GuillotineTesting.unity`
2. Revert if needed: `git checkout HEAD -- "Assets/DLYH/Scenes/GuillotineTesting.unity"`
3. Only commit scene changes intentionally

**24. Progressive Disclosure for Complex UIs**
When a UI screen feels "cluttered", the solution is often UX restructure, not technology change:
- Break all-at-once layouts into wizard steps (2-4 elements per step)
- Use `SetActive()` to completely hide irrelevant sections (not just disable)
- Card-based selection reduces cognitive load vs dropdowns
- Show complex elements (grids, keyboards) only when they become relevant
- Presets eliminate most customization needs for casual users

This pattern applies to any complex setup screen - the technology (uGUI, UI Toolkit, HTML) matters less than the UX design.

### AI-Specific Insights

**25. Grid Density Affects Strategy**
- High density (>35%): Favor coordinate guessing
- Medium density (12-35%): Balanced approach
- Low density (<12%): Favor letter guessing

**26. Rubber-Banding Needs Bounds Testing**
Test with intentionally poor play to find edge cases. Systems can over-correct without proper bounds.

**27. Adaptive Thresholds**
...
## Bug Fix History (December 22, 2025)

| Bug | Root Cause | Fix |
|-----|------------|-----|
| "heroin" appearing in word list | Drug words not filtered | Added BANNED_WORDS HashSet to WordBankImporter, removed from words_alpha.txt |
| Board not resetting after game | No reset logic for gameplay state | Added ResetGameplayState() in GameplayUIController, ResetForNewGame() in SetupSettingsPanel |
| Guillotine heads not resetting position | Head position not stored for reset | Added _originalHeadPosition tracking, StoreOriginalHeadPosition() on Initialize |
| Guillotine face not resetting | ResetFace() not called on reset | Added _faceController.ResetFace() call in GuillotineDisplay.Reset() |
| Grid cells overflowing after reset changes | Accidental Unity scene file modifications | Reverted scene file with git checkout |
...
- Drug words added to banned list: heroin, cocaine, meth, crack, weed, opium, morphine, ecstasy, molly, dope, smack, coke
...
| Final Guillotine Raise | Part 1 of execution sequence |
| Hook Unlock | Part 2 of execution sequence |
| Final Guillotine Chop | Part 3 of execution sequence |
| Head Removed | When head falls into basket |

**Design Decisions:**
- Two AudioSources for layered playback
- 3-part execution sequence replaces simple chop sounds
- Static convenience methods for easy calling from GuillotineDisplay

### Head Face Controller - COMPLETE
...
- ResetFace() restores to Neutral expression

**Status:** COMPLETE - All sprites assigned and working
...
## New Systems (December 22, 2025)

### Drug Word Filtering

**Location:** `WordBankImporter.cs`
...
- Words filtered at import time, not runtime

### Board Reset System

**Location:** `GameplayUIController.cs`, `SetupSettingsPanel.cs`, `GuillotineDisplay.cs`

**GameplayUIController.ResetGameplayState():**
- Called at start of `StartGameplay()`
- Clears both guessed word lists
- Resets both letter tracker panels
- Resets both guillotines
- Clears pending game result

**SetupSettingsPanel.ResetForNewGame():**
- Called from `MainMenuController.StartNewGame()`
- Clears player grid placed words
- Resets all word pattern rows
- Reinitializes grid
- Resets player name input

**GuillotineDisplay.Reset():**
- Resets blade to starting position
- Kills active DOTween animations
- Restores head to original position
- Calls `HeadFaceController.ResetFace()`

### Gameplay Settings Button

**Location:** `GameplaySettingsButton.cs`
...
- Added `_openedFromGameplay` flag
- `ShowFromGameplay()` - Sets flag, hides main menu elements
- `OnBackClicked()` - Returns to gameplay or main menu based on flag

### Version Display

**Location:** `VersionDisplay.cs`
...
3. Set version in Project Settings > Player > Version
...
|------|----------|--------|
| Setup screen UX redesign (wizard flow) | UX | TODO (Phase 5) |
...
| Background music (MusicManager) | Audio | COMPLETE |
| Dynamic music tempo | Audio | COMPLETE |
| Mute buttons during gameplay | UX | COMPLETE |
| Telemetry Dashboard (Editor window) | Development | COMPLETE |
| Enhanced telemetry (player name, game abandon) | Development | COMPLETE |
| MessagePopup positioning (bottom of screen) | UX | COMPLETE |
| Main menu trivia display | UX | COMPLETE |
| Head face expressions | Visual Polish | COMPLETE |
| Final execution audio sequence (3-part) | Audio | COMPLETE |
| Drug word filtering | Content | COMPLETE |
| Board reset on new game | Bug Fix | COMPLETE |
| Settings button during gameplay | UX | COMPLETE |
| Version display on main menu | UX | COMPLETE |
...
**Suggestions:**
1. **Mute buttons during gameplay** - COMPLETE
2. **Background music** - COMPLETE
3. **Dynamic music tempo** - COMPLETE
...
### Remote Playtest Feedback (Dec 22, 2025)

**Bug Reported:**
- "heroin" appeared in word list during gameplay
- Board did not reset after completing a game (old guesses visible)

**Fix:**
- Added drug word filtering with banned words HashSet
- Implemented complete board reset on new game start
...
### Implementation Progress (Dec 17-22, 2025)
...
- MusicManager.cs for background music with shuffle/crossfade
- Mute buttons wired to UIAudioManager and MusicManager
- MessagePopup positioned at bottom of screen
- Main menu trivia display
- Head face sprites assigned in Unity
- 3-part final execution audio sequence
- Drug word filtering
- Board reset on new game
- Settings button during gameplay
...
- HashMarksContainer shortened to leave space at top for final raise
- On execution: 3-part audio sequence (raise, unlock, chop) then blade drops
- Head shakes and falls on game over
- Face changes expression based on danger levels
- All positions properly reset on new game
