...
3. **Invalid Word Feedback System** - ShowFeedback() method backend ready (UI pending)
...
- Invalid words rejected with user-facing message
- Backend ready (ShowFeedback method)
- UI implementation pending (toast/popup)
...
|-------|-------------|----------|
| Grid row labels compression | Row labels don't resize with grid when size changes | LOW (Phase 4) |
| Grid not clearing on size change | If words placed, changing grid size should clear grid | MEDIUM |

### Features to Implement

| Feature | Description | Priority |
|---------|-------------|----------|
| Invalid word feedback UI | Toast/popup for rejected words | HIGH |
| Random word placement | Compass button in grid corner | MEDIUM |
| Rename Forgiveness to Difficulty | Dropdown: Hard/Normal/Easy | LOW |

### Design Considerations

| Item | Notes |
|------|-------|
| Word count vs difficulty | 3 words is HARDER than 4 words - formula accounts for this but UI should clarify |
...
| Component | Status |
|-----------|--------|
| Invalid Word Feedback UI | Backend ready, UI pending |
| Random Word Placement | Not started |
