...
**Total Scripts:** 34 UI + 10 AI = 44 Scripts
...
| `EndPlayerTurn()` | Switch to opponent's turn | void |
...
```
StartGameplay()
    -> CaptureSetupData() for both players
    -> ConfigureOwnerPanel() 
...
| Method | Purpose | Returns |
|--------|---------|---------|
| `ProcessLetterGuess(char)` | Check if letter in words | GuessResult |
| `ProcessCoordinateGuess(row, col)` | Check if cell has letter | GuessResult |
| `ProcessWordGuess(word, rowIndex)` | Validate and check word | GuessResult |
...
**Namespace:** `DLYH.AI.Core`  
**Location:** `Scripts/AI/Core/`  
**Purpose:** Handles AI word selection and grid placement.
...
GuessProcessor.ProcessLetterGuess('E')
    ...
    |
    +---(Miss)---> IncrementMiss(), Check lose condition, End turn
```
...
Fire OnLetterGuess/OnCoordinateGuess/OnWordGuess event
    |
    v
GameplayUIController handles result
    |
    v
RecordAIHit() if successful
```
...
ExecutionerAI
    |
    +--- OnLetterGuess ---> GameplayUIController.ProcessOpponentLetterGuess()
    +--- OnCoordinateGuess ---> GameplayUIController.ProcessOpponentCoordinateGuess()
    +--- OnWordGuess ---> GameplayUIController.ProcessOpponentWordGuess()
```
...
Services receive Actions/Funcs for operations they need but don't own (like incrementing miss count).
