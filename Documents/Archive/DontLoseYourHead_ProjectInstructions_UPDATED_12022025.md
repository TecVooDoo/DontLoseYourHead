...
**Examples:**
- `ProjectInstructions_UPDATED_12032025.md`
- `GDD_UPDATED_12032025.md`
...
- [ ] Invalid word feedback UI (toast/popup) - backend ready
...
| Bug | Fix |
|-----|-----|
| Color picker cycling single button | 8 preset buttons with outline highlight |
| Color not updating visuals | UpdatePlayerColorVisuals() in PlayerGridPanel |
| Word count showing 4 rows on start | InitializeWordRows() in Start() |
| Typing in name field goes to word rows | Focus check in SetupModeController |
| Letter buttons staying grey | Fixed IsInteractable setter in LetterButton |
| 6-letter word list wrong | Reimported via WordBankImporter |
| Word validation not working | ValidateWord + SetWordValidator integration |
...
| Grid not clearing when size changed with words placed | MEDIUM |
| Rename Forgiveness to Difficulty (Hard/Normal/Easy) | LOW |

### Next Development Priorities

**Immediate:**
1. Invalid word feedback UI (toast/popup using Easy Popup System)
2. Random word placement (compass in grid corner)
3. Clear grid when grid size changes (if words placed)

**Soon:**
4. Gameplay Mode UI - Two PlayerGridPanels, turn-based interaction
5. AI opponent system

**Later:**
6. Visual polish (animations, effects)
7. Audio implementation
8. Grid row labels resize fix
...
| Script | Purpose |
|--------|---------|
| PlayerGridPanel.cs | Main grid container, mode switching |
| SetupSettingsPanel.cs | Player config, difficulty, word lists |
...
3. **Easy Popup System** - Popups, toasts, dialogs
