...
**Results Verified:**
- Owner panel shows words fully revealed (e.g., "L O P", "C R A X")
- Opponent panel shows underscores for hidden words
- Unused word rows hide properly (3 words = 3 rows visible)
- Guillotine graphics and miss counters display correctly
...
| Word Pattern Rows | COMPLETE | CacheWordPatternRows() fix (Dec 9) |
| Guillotine Graphics | COMPLETE | Displayed in center area |
| Miss Counters | COMPLETE | Shows 0 / 21 format |
...
### Bug Fixes (Dec 6-9, 2025)

| Bug | Fix Applied | Script(s) Modified |
|-----|-------------|-------------------|
| Word pattern rows blank in gameplay | Added CacheWordPatternRows() before accessing rows | GameplayUIController |
...
### Decision: CacheWordPatternRows() Before Access (Dec 9, 2025)

When accessing word pattern rows on newly activated panels:
- Panels activated before Start() runs
- Word pattern row array not yet cached
- Solution: Call CacheWordPatternRows() explicitly before accessing rows
- Ensures rows are available regardless of Unity lifecycle timing
...
| GameplayUIController | COMPLETE (Dec 9) |
...
| Word Pattern Row Display | COMPLETE (Dec 9) |
| Guillotine Graphics | COMPLETE (Dec 9) |
| Miss Counter Display | COMPLETE (Dec 9) |
...
### 14. Cache Before Access on Inactive Panels (Dec 9, 2025)
- Word pattern rows not cached until Start() runs
- Panels activated before Start() in some workflows
- Always call CacheWordPatternRows() before accessing individual rows
- Don't assume Unity lifecycle methods have run
...
Assets/DLYH/Scripts/
  ...
  UI/
    ...
    GameplayUIController.cs (~400 lines)
