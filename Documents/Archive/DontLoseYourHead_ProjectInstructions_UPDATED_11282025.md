...
**Examples:**
- `ProjectInstructions_UPDATED_11282025.md`
- `GDD_UPDATED_11282025.md`
- `GameManager_UPDATED_11282025.cs`
...
- [ ] Settings panel (Grid Size, Word Count, Forgiveness dropdowns)
- [ ] Word entry with autocomplete integration
- [ ] Coordinate placement mode (yellow/green/red feedback)
- [ ] PlayerGridPanel mode switching (Setup vs Gameplay)

### Next Development Priorities

**Immediate:**
1. **Setup Mode UI** - Settings panel, word entry, coordinate placement
2. **Gameplay Mode UI** - Two PlayerGridPanels, turn-based interaction
3. **Mode Switching** - Transition from Setup to Gameplay
...
The game has two distinct phases with different UI needs:
...
- Player 1 completes setup, then Player 2

**Gameplay Phase (Alternating Turns):**
- Two PlayerGridPanels visible (side-by-side or stacked)
- Center area with guillotines and miss counters
- Click opponent's elements to make guesses
- Turn indicator shows whose turn

### Shared Component: PlayerGridPanel

The same prefab is used for both phases:

```
PlayerGridPanel
  - PlayerNameLabel (Image + Text)
  - WordPatternsContainer (4 word pattern rows)
  - LetterTrackerContainer (A-M row, N-Z row)
  - ColumnLabelsContainer (A through L, hide unused)
  - GridWithRowLabels
    - RowLabelsContainer (1 through 12, hide unused)
    - GridContainer (up to 12x12 cells, hide unused)
```

**Setup Mode Behavior:**
- Letter tracker acts as keyboard (click to type)
- Grid shows own placed words (visible)
- Compass icons active for placement
- Click grid to place letters

**Gameplay Mode Behavior:**
- Letter tracker shows guessed letters only
- Own grid shows own words (visible to self)
- Opponent grid shows guesses only (hidden words)
- Click opponent's elements to guess

### Key UI Components

**Letter Tracker (Dual Purpose):**
- Setup Mode: Click letters to type words
- Gameplay Mode: Shows which letters guessed (highlight hit, dim miss)
- Two rows: A-M and N-Z

**Autocomplete Dropdown:**
- Appears when typing in Setup Mode
- Filters by required word length for current row
- Select from dropdown to complete word entry
- Validates words against word bank

**Coordinate Placement Mode:**
- Activated by compass icon next to word row
- Yellow = current cursor position
- Green = valid positions for next letter
- Red = invalid positions (word won't fit)
- First click places first letter, second click sets direction

**Random Placement:**
- Compass icon in top-left corner of grid
...
**1. Odin Inspector and Validator (v4.0.1.0)**
...
### Tier 2: Development and Data Tools
...
**UI Toolkit Filters, Effects and Shaders**
...
**UI Setup Mode (In Progress):**
- [ ] Settings dropdowns update difficulty values
- [ ] Word entry accepts typed input
- [ ] Autocomplete shows filtered suggestions
- [ ] Autocomplete selection fills word display
- [ ] Coordinate mode shows yellow/green/red feedback
- [ ] First click places first letter
- [ ] Second click determines direction
- [ ] Random placement works correctly
- [ ] All words required before Start enabled

**UI Gameplay Mode (TODO):**
- [ ] Both grids display correctly
- [ ] Letter click on opponent grid registers guess
- [ ] Coordinate click on opponent grid registers guess
- [ ] Hit/miss visual feedback works
...
- [X] Reuse PlayerGridPanel for both Setup and Gameplay modes
