...
|-----------|---------|---------|
| `TecVooDoo.DontLoseYourHead.UI` | 28 | Main UI scripts (including new UI system) |
...
|
+-- Editor/
|   +-- TelemetryDashboard.cs           (~920 lines)
|
+-- Telemetry/
|   +-- PlaytestTelemetry.cs            (~460 lines)
|
+-- Core/
...
|
+-- UI/
    |-- GameplayUIController.cs         (~2,150 lines)
    ...
    |-- LetterCellUI.cs                 (~543 lines) [NEW]
    |-- MainMenuController.cs           (~455 lines)
    |-- MessagePopup.cs                 (~450 lines)
    ...
    |-- WordPatternRowUI.cs             (~655 lines) [NEW]
...
**Features:**
- Calls `PlaytestTelemetry.SetTurnNumber()` after each turn
- Calls `MessagePopup.ResetPosition()` at game start
- Game abandon tracking in `ReturnToSetup()`
...
Located in `Assets/DLYH/NewUI/` and `Assets/DLYH/Scripts/UI/`:

| Component | Lines | Purpose |
|-----------|-------|---------|
| LetterCellUI | ~543 | Unified cell for letters, icons, animations |
| WordPatternRowUI | ~655 | Row manager with 12 unified cells |

**LetterCellUI Features:**
- Content types: Letter, Icon, Empty
- States: Default, Selected, Revealed, Locked, Disabled, Highlighted
- Animations: Spin, Punch, Scale, Shake, Color, Fade, Reveal
- Player color theming for revealed letters

**WordPatternRowUI Structure:**
- Cell 0: Row label (1-5)
- Cells 1-8: Letter cells (hide unused based on word length)
- Cells 9-11: Action cells (Setup: Select/Place/Delete, Gameplay: GuessWord)

See `DLYH_NewUI_Architecture_01042026.md` for complete technical documentation.

### Legacy UI Components

| Component | Lines | Purpose |
|-----------|-------|---------|
| WordPatternRow | ~1,199 | Individual word entry row (legacy) |
...
| MessagePopup | ~450 | Turn notifications with position memory |
...
### 14. Unified Cell Pattern (New)
All elements in a row use the same cell component for uniform appearance and animation.

```csharp
// WordPatternRowUI creates 12 identical LetterCellUI instances
// Cell 0: Row label
// Cells 1-8: Letter cells
// Cells 9-11: Action cells
// Each cell can display letter OR icon based on ContentType
```
...
WordPatternRowUI (New)
    +--- OnLetterCellClicked ---> Parent handler
    +--- OnSelectClicked ---> Coordinate placement mode
    +--- OnPlaceClicked ---> Place word on grid
    +--- OnDeleteClicked ---> Clear word
    +--- OnGuessWordClicked ---> Word guess mode
```
