...
**Examples:**
- `ProjectInstructions_v3_UPDATED_12132025.md`
- `GDD_v3_UPDATED_12132025.md`
- `DESIGN_DECISIONS_v3_UPDATED_12152025.md`
- `DLYH_Architecture_v3_UPDATED_12132025.md`
...
**Interfaces:** Used for interchangeable strategies and controller contracts.
- IGuessStrategy: Allows swapping letter/coordinate/word strategies
- IGridControllers: Contracts for extracted controllers (5 interfaces)
...
## AI System Architecture (Phase 3 - INTEGRATED & PLAYTESTED)
...
Assets/DLYH/
  ...
  Scripts/
    AI/
      ...
      Core/
        ExecutionerAI.cs             (~493 lines) - Main MonoBehaviour
        DifficultyAdapter.cs         (~268 lines) - Rubber-banding
        MemoryManager.cs             (~442 lines) - Skill-based memory
        AISetupManager.cs            (~468 lines) - Word selection/placement
      Strategies/
        IGuessStrategy.cs            (~493 lines) - Interface + data structures
        LetterGuessStrategy.cs       (~327 lines)
        CoordinateGuessStrategy.cs   (~262 lines)
        WordGuessStrategy.cs         (~327 lines)
      Data/
        LetterFrequency.cs           (~442 lines) - Static frequency data
        GridAnalyzer.cs              (~442 lines) - Fill ratio calculations
```

### Key Classes

**ExecutionerConfigSO:** ScriptableObject with all tunable parameters.
- Rubber-banding settings (skill bounds 0.15-0.95, adjustment step 0.15)
- Initial skill per difficulty: Easy=0.25, Normal=0.50, Hard=0.75
- Adaptive threshold settings (bounds 1-7, trigger after 2 consecutive)
- Strategy preferences (density thresholds 0.35/0.12, word guess risk 0.7)
- Memory settings (30% max forget chance, always remember 3 recent)
- Think time range (0.8-2.5 seconds)

**ExecutionerAI:** MonoBehaviour that coordinates turn execution.
- Owns DifficultyAdapter, MemoryManager, and three strategy instances
- Manages async think time via UniTask
- Fires events: OnLetterGuess, OnCoordinateGuess, OnWordGuess
- Inverts player difficulty (Player Easy = AI Hard starting skill)

**DifficultyAdapter:** Plain C# class tracking runtime difficulty state.
- Current skill level (0.15-0.95)
- Current hit/miss thresholds (adapt over time)
- Recent player guess history (queue of bool)
- Consecutive adjustment tracking for threshold adaptation
- Methods: RecordPlayerGuess(), IncreaseSkill(), DecreaseSkill()
...
public interface IGuessStrategy
{
    ...
    GuessRecommendation Evaluate(AIGameState state, float skillLevel, ExecutionerConfigSO config);
}
```

**AIGameState:** Data class containing all info strategies need.
```csharp
public class AIGameState
{
    public int GridSize;
    public HashSet<char> GuessedLetters;
    public HashSet<(int, int)> HitCoordinates;
    public HashSet<(int, int)> MissCoordinates;
    public List<string> WordPatterns;  // "C_T", "____", etc.
    public WordListSO[] WordLists;
}
```
...
5. **Memory filtering** - Lower skill AI "forgets" older information

### AI Integration (COMPLETE - Dec 13, 2025)

All AI integration tasks completed:

| Task | Status |
|------|--------|
| WireAIEvents() in GameplayUIController | COMPLETE |
| HandleAILetterGuess() handler | COMPLETE |
| HandleAICoordinateGuess() handler | COMPLETE |
| HandleAIWordGuess() handler | COMPLETE |
| BuildAIGameState() method | COMPLETE |
...
| 16+ compilation errors fixed | COMPLETE |
...
- [X] WordPatternController (IWordPatternController implementation)

**Interfaces (IGridControllers.cs):**
- [X] IGridDisplayController
- [X] ILetterTrackerController
- [X] IWordPatternController
- [X] ICoordinatePlacementController
- [X] IGridColorManager
- [X] GridHighlightType enum
- [X] PlacementState enum
...
- [X] IGridControllers.cs interfaces documented
...
- [X] ExecutionerConfigSO (tunable parameters)
- [X] ExecutionerAI controller (MonoBehaviour)
- [X] DifficultyAdapter (rubber-banding + adaptive thresholds)
- [X] MemoryManager (skill-based filtering)
- [X] AISetupManager (word selection/placement)
- [X] IGuessStrategy + AIGameState + GuessRecommendation
- [X] LetterGuessStrategy (frequency + pattern analysis)
- [X] CoordinateGuessStrategy (adjacency + density awareness)
- [X] WordGuessStrategy (confidence thresholds)
- [X] LetterFrequency static data
- [X] GridAnalyzer utility
- [X] Wire ExecutionerAI events to GameplayUIController
- [X] BuildAIGameState() method
- [X] TriggerAITurn() after player turn ends
- [X] RecordPlayerGuess() for rubber-banding
- [X] AISetupManager replaces GenerateOpponentData()
- [X] Win condition checking
- [X] First playtest with real user (Stacey)

**Playtest Bug Fixes (IN PROGRESS - Dec 13, 2025):**
- [ ] Letter tracker not green on word guess (both panels)
- [ ] Center panel player names wrong ("Player 1/2" instead of actual names)
- [ ] Wrong word guess counts 1 miss instead of 2
- [X] Auto-hide Guess Word buttons when words fully revealed via letters
...
**Phase 3: Playtest Bug Fixes (Current)**
1. Fix letter tracker update on correct word guess
2. Fix center panel player name labels
3. Fix wrong word guess miss count (Action -> Action<int>)

**Phase 3: Remaining Polish**
4. Win/Lose UI feedback
5. Turn indicator improvements

### Phase 4: Polish and Features

6. Visual polish (DOTween animations, Feel effects)
7. Audio implementation
8. Invalid word feedback UI (toast/popup)
9. Profanity filter for word bank
10. Medieval/carnival themed monospace font

### Phase 5: Multiplayer and Mobile

11. 2-player networking mode (human vs human online)
...
| GameplayUIController.cs | Two-panel gameplay system | ~1,250 | AI methods added Dec 13 |
...
| WordPatternController.cs | IWordPatternController implementation | Controllers/ |
...
| GridCellManager.cs | Cell array management | Controllers/ |
...
### Interface Contracts (IGridControllers.cs)

| Interface | Purpose | Implementations |
|-----------|---------|-----------------|
| IGridDisplayController | Grid display operations | GridLayoutManager, GridCellManager |
| ILetterTrackerController | Letter tracker operations | LetterTrackerController |
| IWordPatternController | Word pattern row management | WordPatternController |
| ICoordinatePlacementController | Placement mode operations | CoordinatePlacementController |
| IGridColorManager | Color/highlighting operations | GridColorManager |
...
- AI configuration as ScriptableObject (ExecutionerConfigSO)
...
{
    private readonly Action<int> _onMissIncrement;  // Pass miss count
...
**Note:** Use `Action<int>` instead of `Action` when the callback needs to pass variable data like miss counts.
...
| DontLoseYourHead_ProjectInstructions | Development protocols, MCP tools | v3.1 |
| DESIGN_DECISIONS | Technical decisions, lessons learned | v3.1 |
...
- [X] Use Action<int> instead of Action when callbacks need to pass variable data
- [X] Playtest with real users, not just developer testing
- [X] Auto-hide UI elements when they become irrelevant (e.g., Guess Word on fully revealed words)
...
- [ ] Use Action when Action<T> is needed for passing variable data
