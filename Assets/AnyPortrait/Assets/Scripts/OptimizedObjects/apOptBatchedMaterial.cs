...
namespace AnyPortrait
{
	...
	public class apOptBatchedMaterial
	{
		...
		public class MaterialUnit
		{
			...
			//v1.6.2 : 버그
			//생성은 되었지만 이 재질과 관련된 모든 메시가 Clipped 되는 등의 이유로 참조가 되지 않는다면 Material이 생성되지 않는다.
			//이 경우, 이 재질 정보는 사용될 수 없다. (Null 에러 발생)
			[NonSerialized] private bool _isInitialized = false;
			

			public MaterialUnit()
			{
				if(_linkedMeshes == null) { _linkedMeshes = new List<apOptMesh>(); }
				_linkedMeshes.Clear();
			...
			{
				if(_linkedMeshes == null)
				{
					_linkedMeshes = new List<apOptMesh>();
				}
			...
			public void ResetRequestProperties()
			{
				...
				int nLinkedMeshes = _linkedMeshes != null ? _linkedMeshes.Count : 0;
				if (nLinkedMeshes == 0)
				{
					return;
				}
			...
			//변경 v1.6.2 : Texture2D > Texture
			/// <summary>
			/// 임의의 프로퍼티의 텍스쳐 속성을 변경한다. 호출 즉시 항상 Batched Material을 사용하게 된다.
			/// </summary>
			public void RequestCustomImage(Texture texture, string propertyName)
			{
				//v1.6.2 : 초기화 되지 않은 상태 체크
				if(!_isInitialized)
				{
			...
			//변경 v1.6.2 : Texture2D > Texture
			//ID를 사용한 버전 [v1.4.3]
			public void RequestCustomImage(Texture texture, int propertyNameID)
			{
				//v1.6.2 : 초기화 되지 않은 상태 체크
				if(!_isInitialized)
				{
		...
		public void ResetAllProperties()
		{
			...
			if(nClipped > 0)
			{
				ClippedMatMeshSet curClippedUnit = null;
				for (int i = 0; i < nClipped; i++)
				{
					curClippedUnit = _clippedMatUnits[i];
					curClippedUnit.ResetRequestProperties();
				}
			}
		}
		...
		public void ResetProperties(int textureID)
		{
			...
			if(nClipped > 0)
			{
				ClippedMatMeshSet curClipped = null;
				for (int i = 0; i < nClipped; i++)
				{
					curClipped = _clippedMatUnits[i];
					if(!curClipped.IsValid())
					{
						continue;
					}
					if(curClipped._matInfo._textureID != textureID)
					{
						continue;
					}

					curClipped.ResetRequestProperties();
				}
			}
		}
		...
		public void SetMeshImageAll(int textureID, Texture2D newTexture)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_Texture(newTexture);
				}
			}
		}
		...
		public void SetMeshColorAll(int textureID, Color color)
		{
			...
			{			
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_Color(color);
				}
			}
		}
		...
		public void SetMeshAlphaAll(int textureID, float alpha)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_Alpha(alpha);
				}
			}
		}
		...
		//변경 v1.6.2 : Texture2D > Texture
		public void SetMeshCustomImageAll(int textureID, Texture newTexture, string propertyName)
		{
			MaterialUnit curUnit = null;
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomTexture(newTexture, propertyName);
				}
			}
		}
		...
		//ID를 사용한 버전 [v1.4.3]
		//변경 v1.6.2 : Texture2D > Texture
		public void SetMeshCustomImageAll(int textureID, Texture newTexture, int propertyNameID)
		{
			MaterialUnit curUnit = null;
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomTexture(newTexture, propertyNameID);
				}
			}
		}
		...
		//추가 22.1.9 : 텍스쳐 ID를 안받고 모두 적용
		//변경 v1.6.2 : Texture2D > Texture
		public void SetMeshCustomImageAll_WithoutTextureID(Texture newTexture, string propertyName)
		{
			int nMatUnits = _matUnits != null ? _matUnits.Count : 0;
			if (nMatUnits > 0)
			{
		...
		//ID를 사용한 버전 [v1.4.3]
		//변경 v1.6.2 : Texture2D > Texture
		public void SetMeshCustomImageAll_WithoutTextureID(Texture newTexture, int propertyNameID)
		{
			int nMatUnits = _matUnits != null ? _matUnits.Count : 0;
			if (nMatUnits > 0)
			{
		...
		public void SetMeshCustomImageOffsetAll_WithoutTextureID(Vector2 offset, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomTextureOffset(offset, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomImageScaleAll_WithoutTextureID(Vector2 scale, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomTextureScale(scale, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomColorAll(int textureID, Color color, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomColor(color, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomColorAll(int textureID, Color color, int propertyNameID)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomColor(color, propertyNameID);
				}
			}
		}
		...
		public void SetMeshCustomAlphaAll(int textureID, float alpha, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomAlpha(alpha, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomAlphaAll(int textureID, float alpha, int propertyNameID)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomAlpha(alpha, propertyNameID);
				}
			}
		}
		...
		public void SetMeshCustomFloatAll(int textureID, float floatValue, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomFloat(floatValue, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomFloatAll(int textureID, float floatValue, int propertyNameID)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomFloat(floatValue, propertyNameID);
				}
			}
		}
		...
		public void SetMeshCustomIntAll(int textureID, int intValue, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomInt(intValue, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomIntAll(int textureID, int intValue, int propertyNameID)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomInt(intValue, propertyNameID);
				}
			}
		}
		...
		public void SetMeshCustomVector4All(int textureID, Vector4 vecValue, string propertyName)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomVector4(vecValue, propertyName);
				}
			}
		}
		...
		public void SetMeshCustomVector4All(int textureID, Vector4 vecValue, int propertyNameID)
		{
			...
			if(nClipped > 0)
			{
				...
				for (int i = 0; i < nClipped; i++)
				{
					...
					curClipped._clippedMesh.SetClippedMaterialPropertyByBatch_CustomVector4(vecValue, propertyNameID);
				}
			}
		}
