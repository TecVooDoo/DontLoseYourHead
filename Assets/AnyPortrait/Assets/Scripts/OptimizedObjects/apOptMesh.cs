...
namespace AnyPortrait
{
	...
	public class apOptMesh : MonoBehaviour
	{
		...
		//private bool _cal_isVisibleRequest_Masked = false;//삭제 v1.6.1
...
// 			}
		...
		public void UpdateVisibility(bool isVisible)
		{
			...
			//v1.6.1 버그 : 무조건 Parent가 먼저 연산이 된다는 보장이 없으므로, 잘못하면 Parent의 Visible이 계산되기 전에 이게 먼저 호출될 수 있다.
			//그러면 "이전 프레임"에서의 "Parent Visible" 값을 참조하게 된다.
			//따라서 

			//추가 > 버그. 코드 위치 이동. 이 부분은 호출 순서를 맞추기 위해 UpdateCalculate()에서 Visible을 계산한다.
										...
										bool isOrthoCorrection)
		{
			...
			{
				if (!_parentOptMesh._isVisible)
				{
					_cal_isVisibleRequest = false;
				}
			}

			if(_cal_isVisibleRequest)
			{
				if(!_isVisible)
				{
		...
		public void SyncMaterialPropertyByBatch_CustomTexture(Texture texture, string propertyName)
		{
		...
		//변경 v1.6.2 : Texture2D > Texture
		public void SyncMaterialPropertyByBatch_CustomTexture(Texture texture, int propertyNameID)//ID를 사용한 버전 [v1.4.3]
		{
		...
		//[v1.6.2 변경] Texture2D > Texture
		/// <summary>
		/// Set Texture as shader property (not Main Texture)
		/// </summary>
		/// <param name="texture"></param>
		/// <param name="propertyName"></param>
		public void SetCustomTexture(Texture texture, string propertyName)
		{
			//값에 상관없이 이 함수가 호출되면 True
			_isAnyCustomPropertyRequest = true;
		...
		/// Set Texture as shader property (not Main Texture)
		/// </summary>
		public void SetCustomTexture(Texture texture, int propertyNameID)//ID 버전 [v1.4.3]
		{
			//값에 상관없이 이 함수가 호출되면 True
			_isAnyCustomPropertyRequest = true;
			_material_Instanced.SetTexture(propertyNameID, texture);
		...
		//변경 v1.6.2 : Texture2D > Texture
		public void SetClippedMaterialPropertyByBatch_CustomTexture(Texture texture, string propertyName)
		{
			//if(!_isMaskChild)
			if(!IsMaskClipped())//v1.6.0
			{
		...
		//ID를 사용한 버전 [v1.4.3]
		//변경 v1.6.2 : Texture2D > Texture
		public void SetClippedMaterialPropertyByBatch_CustomTexture(Texture texture, int propertyNameID)
		{
			//if(!_isMaskChild)
			if(!IsMaskClipped())//v1.6.0
			{
		...
		//변경 v1.6.2 : Texture2D > Texture
		public void SyncMergedMaterial_CustomImage(Texture texture, ref string propertyName)
		{
			//Instanced의 값을 동기화한다.
			//Merged는 유지하도록 한다.
		...
		//ID를 사용한 버전 [v1.4.3]
		//변경 v1.6.2 : Texture2D > Texture
		public void SyncMergedMaterial_CustomImage(Texture texture, int propertyNameID)
		{
			_material_Instanced.SetTexture(propertyNameID, texture);
		}
