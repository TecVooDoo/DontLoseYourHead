	...
	public class apOptMaterialInfo
	{
							...
							apMaterialLibrary materialLibrary)
		{
			...
			if(_shader != null)
			{
				...
				int nShaderProps = _shader.GetPropertyCount();
#else
				int nShaderProps = ShaderUtil.GetPropertyCount(_shader);//6.2에서 Obsolete
#endif
				...
				if (nShaderProps > 0)
				{
					...
					{
#if UNITY_6000_2_OR_NEWER
						string propName = _shader.GetPropertyName(iProp);
#else
						string propName = ShaderUtil.GetPropertyName(_shader, iProp);//6.2에서 Obsolete
#endif
			...
			{
#if UNITY_6000_2_OR_NEWER
				int nRefProps = refShader.GetPropertyCount();
#else
				int nRefProps = ShaderUtil.GetPropertyCount(refShader);//6.2에서 Obsolete
#endif
				...
				if (nRefProps > 0)
				{
					for (int iRefProp = 0; iRefProp < nRefProps; iRefProp++)
					{
...
#if UNITY_6000_2_OR_NEWER
						string refPropName = refShader.GetPropertyName(iRefProp);
#else
						string refPropName = ShaderUtil.GetPropertyName(refShader, iRefProp);//6.2에서 Obsolete
#endif
						...
						//타입에 따라 재질의 값을 가져오자
#if UNITY_6000_2_OR_NEWER
						UnityEngine.Rendering.ShaderPropertyType refPropType = refShader.GetPropertyType(iRefProp);
#else
						ShaderUtil.ShaderPropertyType refPropType = ShaderUtil.GetPropertyType(refShader, iRefProp);//6.2에서 Obsolete
#endif
