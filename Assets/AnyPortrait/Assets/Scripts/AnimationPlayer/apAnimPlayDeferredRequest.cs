...
namespace AnyPortrait
{
	...
	public class apAnimPlayDeferredRequest
	{
		...
		public class CommandUnit
		{
			...
			public bool _isPauseAfterFrameChanged = false;//ChangeFrame에서 Pause 여부 (v1.6.1 추가)
			...
			public void PauseAt(	apAnimPlayData playData,
									int frame,
									int layer,
									apAnimPlayUnit.BLEND_METHOD blendMethod,
									apAnimPlayManager.PLAY_OPTION playOption,
									bool isAutoEndIfNotloop)
			{
				_commandType = COMMAND_TYPE.PauseAt;
				_isValid = true;
			...
			public void ChangeFrame(	apAnimPlayData playData,
										int frame,
										bool isPauseAfterFrameChanged)
			{
				_commandType = COMMAND_TYPE.ChangeFrame;
				_isValid = true;
				_playData = playData;
				_frame = frame;
				_isPauseAfterFrameChanged = isPauseAfterFrameChanged;//v1.6.1 : Pause 여부를 추가한다.
			}
		...
		public void PauseAt(	apAnimPlayData playData,
							...
							bool isAutoEndIfNotloop)
		{
			GetNextCommand().PauseAt(playData, frame, layer, blendMethod, playOption, isAutoEndIfNotloop);
		}
		...
		public void ChangeFrame(	apAnimPlayData playData,
									...
									bool isPauseAfterFrameChanged)
		{
			GetNextCommand().ChangeFrame(playData, frame, isPauseAfterFrameChanged);
		}
		...
		private void Validate()
		{
			...
			for (int i = _iCommand - 1; i >= 0; i--)
			{
				...
				switch (_curCmd._commandType)
				{
					...
					case COMMAND_TYPE.PauseAt://v1.6.1 : 이것도 재생으로 간주해서 처리한다.
						{
							//일반 Play 계열
							...
							{
								//Root Unit이 없다면 할당 (이 요청의 Root Unit으로 전환될 요청이다)
							...
							{
								//v1.4.0에서 고쳐진 버그
								//이미 바로 위 if 문에서 Root Unit 전환 요청이 있었는데, 다른 Root Unit으로의 전환을 다시 요청할 수 없다.
					...
					case COMMAND_TYPE.ChangeFrame:
						{
							//Change Frame은 Root Unit과 무관하게 요청이 들어온 순간의 애니메이션의 재생 여부를 체크할 것이므로
							//PlayData 유효성만 체크한다.]
							if(_curCmd._playData == null)
							{
								//알 수 없는 에러가 발생했다.
		...
		private void ExecuteCommands()
		{
			...
			for (int i = 0; i < _iCommand; i++)
			{
				...
				switch (_curCmd._commandType)
				{
					...
					case COMMAND_TYPE.PauseAt:
						{
							_animPlayManager.PauseAt(	_curCmd._playData,
														_curCmd._frame,
														_curCmd._layer,
														_curCmd._blendMethod,
														_curCmd._playOption,
					...
					case COMMAND_TYPE.ChangeFrame:
						{
							_animPlayManager.ChangeFrame(	_curCmd._playData,
															_curCmd._frame,
															_curCmd._isPauseAfterFrameChanged);
						}
						break;
				}
			}
