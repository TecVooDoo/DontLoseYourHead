...
namespace AnyPortrait
{
	...
	public class apAnimPlayManager
	{
		...
		public apAnimPlayData Play(apAnimPlayData playData, int layer, apAnimPlayUnit.BLEND_METHOD blendMethod, PLAY_OPTION playOption = PLAY_OPTION.StopSameLayer, bool isAutoEndIfNotloop = false, bool isDebugMsg = true)
		{
			...
			{
				//다른 레이어를 모두 정지시킨다.
				int nQueueCount = _animPlayQueues.Count;
				if (nQueueCount > 0)
				{
		...
		//추가 v1.6.1
		public apAnimPlayData PauseAt(apAnimPlayData playData, int frame, int layer, apAnimPlayUnit.BLEND_METHOD blendMethod, PLAY_OPTION playOption = PLAY_OPTION.StopSameLayer, bool isAutoEndIfNotloop = false)
		{
			if (playData == null)
			{
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : PauseAt Failed : Unknown AnimPlayData");
#endif
				return null;
			}
			...
			{
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : PauseAt Failed : Layer " + layer + " is invalid. Layer must be between " + MIN_LAYER_INDEX + " ~ " + MAX_LAYER_INDEX);
#endif
				return null;
			}

			apAnimPlayQueue playQueue = _animPlayQueues[layer];
			apAnimPlayUnit resultPlayUnit = playQueue.PauseAt(playData, frame, blendMethod, isAutoEndIfNotloop);
		...
		public apAnimPlayData ChangeFrame(apAnimPlayData playData, int frame, bool isPauseAfterFrameChanged)
		{
			if (playData == null)
			{
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : ChangeFrame Failed : Unknown AnimPlayData");
#endif
				return null;
			}

			if (playData.PlaybackStatus == apAnimPlayData.AnimationPlaybackStatus.None)
			{
				//아직 재생이 안되었다.
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : ChangeFrame Failed : Animation clip is not playing");
#endif
				return null;
			}
			if (playData._linkedAnimClip == null)
			{
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : ChangeFrame Failed : Animation data error");
#endif
				return null;
			}
			apAnimPlayUnit playUnit = playData._linkedAnimClip._parentPlayUnit;
			if (playUnit == null)
			{
#if UNITY_EDITOR
				Debug.LogError("AnyPortrait : ChangeFrame Failed : Animation data error");
			...
			if(playData.PlaybackStatus == apAnimPlayData.AnimationPlaybackStatus.Paused)
			{
				//Paused 상태라면 > 인자에 상관없이 계속 Pause
				playUnit.ResetPauseAt(frame);
			}
			else
			{
				//재생 중이었다면
				if(isPauseAfterFrameChanged)
				{
					//이동 후 Pause를 한다.
					playUnit.ResetPauseAt(frame);
				}
				...
				{
					//Pause 상태가 아니라면 > Pause를 해제하고, 프레임을 변경한다.
					playUnit.ResetPlayAt(frame);
				}
			}
