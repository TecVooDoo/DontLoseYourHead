...
namespace AnyPortrait
{
	...
	public class apAnimPlayQueue
	{
		...
		//추가 1.6.1 : PlayAt과 유사하지만, 프레임 지정 직후 Pause를 한다.
		public apAnimPlayUnit PauseAt(apAnimPlayData playData, int frame, apAnimPlayUnit.BLEND_METHOD blendMethod, bool isAutoEndIfNotloop = true)
		{
			//현재 상태에서 실행되지 않은 Queued 애니메이션 재생 요청은 삭제한다.
			PushAllNoActiveRequests();
			...
			//newPlayUnit.Resume();//<<Pause At에서는 Resume을 풀지 않는다.

			//Play 명령을 준다.
			request.PauseNewAt(newPlayUnit, frame);//<<수정됨
