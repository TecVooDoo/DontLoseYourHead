...
namespace AnyPortrait
{
	...
	public class apBackup
	{
							...
							case apBackupUnit.FIELD_CATEGORY.Instance:
								{
									...
									if (unitType == null)
									{
										//v1.6.2 : 버전이 나뉘게 되면서, DLL 내의 타입의 파싱이 실패할 수도 있다.
										string correctedTypeName = "";
										bool isCorrectedDLLType = CheckDLLType(curUnit._typeName_Partial, out correctedTypeName);
										if(isCorrectedDLLType)
										{
											//다시 한번 더 파싱한다.
											unitType = System.Type.GetType(correctedTypeName);
										}
									}
							...
							case apBackupUnit.FIELD_CATEGORY.Array:
								{
									...
									try
									{
										...
										{
											isList = true;
											System.Type listType = System.Type.GetType(curUnit._typeName_Partial);
											if(listType == null)
											{
												string correctedTypeName = "";
												bool isCorrectedDLLType = CheckDLLType(curUnit._typeName_Partial, out correctedTypeName);
												...
												{
													//다시 한번 더 파싱한다.
													listType = System.Type.GetType(correctedTypeName);
												}
											}

											listObject = System.Activator.CreateInstance(listType) as IList;
										}
										else
										{
											isList = false;
											...
											if(arrayType == null)
											{
												...
												{
													//다시 한번 더 파싱한다.
													arrayType = System.Type.GetType(correctedTypeName);
												}
											}
		...
		/// <summary>
		/// [v1.6.2] 파싱 중에 DLL 내의 클래스 타입이 맞지 않는 경우, 한번 수정해볼 필요는 있다.
		/// 타입의 경로 중에 DLL 이름이 있는 경우에만 유효하며, 그 경우 true를 리턴한다.
		/// 현재 빌드 방식에 따라 변환 내용이 결정된다.
		/// </summary>
		/// <param name="srcTypeNamePartial"></param>
		/// <param name="correctedTypeName"></param>
		/// <returns></returns>
		private bool CheckDLLType(string srcTypeNamePartial, out string correctedTypeName)
		{
			//AnyPortrait_FULL
			//AnyPortrait_FULL_Editor
			//AnyPortrait_FULL_CN
			//AnyPortrait_FULL_CN_Editor
			//AnyPortrait_DEMO
			//AnyPortrait_DEMO_Editor
			//AnyPortrait_DEMO_CN
			//AnyPortrait_DEMO_CN_Editor

			string strDLLName_Editor = "";
			string strDLLName_Runtime = "";
			switch (apVersion.I.BuildType)
			{
				case apVersion.BUILD_TYPE.Global_Full:
					strDLLName_Editor =		"AnyPortrait_FULL_Editor";
					strDLLName_Runtime =	"AnyPortrait_FULL";
					break;

				case apVersion.BUILD_TYPE.Global_Demo:
					strDLLName_Editor =		"AnyPortrait_DEMO_Editor";
					strDLLName_Runtime =	"AnyPortrait_DEMO";
					break;

				case apVersion.BUILD_TYPE.China_Full:
					strDLLName_Editor =		"AnyPortrait_FULL_CN_Editor";
					strDLLName_Runtime =	"AnyPortrait_FULL_CN";
					break;

				case apVersion.BUILD_TYPE.China_Demo:
					strDLLName_Editor =		"AnyPortrait_DEMO_CN_Editor";
					strDLLName_Runtime =	"AnyPortrait_DEMO_CN";
					break;
			}

			//에디터 먼저 체크
			if(srcTypeNamePartial.Contains("AnyPortrait_FULL_Editor"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_FULL_Editor", strDLLName_Editor);
				return true;
			}
			
			if(srcTypeNamePartial.Contains("AnyPortrait_FULL_CN_Editor"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_FULL_CN_Editor", strDLLName_Editor);
				return true;
			}

			if(srcTypeNamePartial.Contains("AnyPortrait_DEMO_Editor"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_DEMO_Editor", strDLLName_Editor);
				return true;
			}

			if(srcTypeNamePartial.Contains("AnyPortrait_DEMO_CN_Editor"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_DEMO_CN_Editor", strDLLName_Editor);
				return true;
			}


			//런타임 DLL 경로 변경
			if(srcTypeNamePartial.Contains("AnyPortrait_FULL"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_FULL", strDLLName_Runtime);
				return true;
			}

			if(srcTypeNamePartial.Contains("AnyPortrait_FULL_CN"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_FULL_CN", strDLLName_Runtime);
				return true;
			}

			if(srcTypeNamePartial.Contains("AnyPortrait_DEMO"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_DEMO", strDLLName_Runtime);
				return true;
			}

			if(srcTypeNamePartial.Contains("AnyPortrait_DEMO_CN"))
			{
				correctedTypeName = srcTypeNamePartial.Replace("AnyPortrait_DEMO_CN", strDLLName_Runtime);
				return true;
			}

			correctedTypeName = srcTypeNamePartial;
			return false;
		}
