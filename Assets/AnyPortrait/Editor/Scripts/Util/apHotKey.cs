	...
	public class apHotKey
	{
		...
		public HotKeyCheck CheckHotKeyEvent(KeyCode keyCode, bool isShift, bool isAlt, bool isCtrl, bool isPressed)
		{
			...
			if (_nEvent_woCS > 0 
				&& !isAlt				
				&& isPressed
				//v1.6.2 버그 수정 : Ctrl/Shift 무관 단축키의 경우, Ctrl/Shift가 있는 유니티 에디터 단축키와 충돌되는 경우에 사용자에게 혼란을 줄 수 있다.
				//그래서 이 단축키 (대부분 기즈모 QWER)는 원래 Ctrl/Shift가 눌려도 동작하지만, 여기서는 안눌러야 동작하게 된다.
				&& !isCtrl
				&& !isShift
				)
			{
