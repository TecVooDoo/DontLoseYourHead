...
namespace AnyPortrait
{
	...
	public partial class apSelection
	{
		...
		private void DrawMeshProperty_Modify(int width, int height)
		{
			...
			if (isSingleVertex)
			{
				apVertex curVert = Editor.VertController.Vertex;
				if (curVert != null
					&& Editor.IsDelayedGUIVisible(apEditor.DELAYED_UI_TYPE.Mesh_Property_Modify_UI_Single))//"Mesh Property Modify UI Single"
				{
					if (_strWrapper_64 == null)
					{
						_strWrapper_64 = new apStringWrapper(64);
					...
					_strWrapper_64.Append(curVert._index, true);
					...
					Vector2 prevPos2 = curVert._pos;
					Vector2 nextPos2 = apEditorUtil.DelayedVector2Field(curVert._pos, width - 10);

					GUILayout.Space(5);

					EditorGUILayout.LabelField(apStringFactory.I.UV);//"UV"
					Vector2 prevUV = curVert._uv;
					Vector2 nextUV = apEditorUtil.DelayedVector2Field(curVert._uv, width - 10);
					...
					float prevDepth = curVert._zDepth;
					float nextDepth = EditorGUILayout.Slider(curVert._zDepth, 0.0f, 1.0f, apGUILOFactory.I.Width(width));

					if(EditorGUI.EndChangeCheck())
					{
						if (Mathf.Abs(nextPos2.x - prevPos2.x) > 0.0001f ||
							Mathf.Abs(nextPos2.y - prevPos2.y) > 0.0001f ||
							Mathf.Abs(nextUV.x - prevUV.x) > 0.0001f ||
							Mathf.Abs(nextUV.y - prevUV.y) > 0.0001f ||
							Mathf.Abs(nextDepth - prevDepth) > 0.0001f)
						{
							//Vertex 정보가 바뀌었다.
							...
							curVert._pos = nextPos2;
							curVert._uv = nextUV;
							curVert._zDepth = nextDepth;

							Mesh.RefreshVertexAutoUV(curVert);
		...
		private void DrawMeshProperty_Pivot(int width, int height)
		{
			...
			EditorGUILayout.LabelField(Editor.GetUIWord(UIWORD.Position));//"Position"
			EditorGUI.BeginChangeCheck();
			Vector2 prevPos2 = mesh._offsetPos;
			Vector2 nextPos2 = apEditorUtil.DelayedVector2Field(mesh._offsetPos, width - 10);
			if(EditorGUI.EndChangeCheck())
			{
				if(Mathf.Abs(prevPos2.x - nextPos2.x) > 0.0001f
					|| Mathf.Abs(prevPos2.y - nextPos2.y) > 0.0001f)
				{
					Editor.Controller.SetMeshPivot(mesh, nextPos2);
				}
			}
			...
			//"Reset Pivot"
			if (GUILayout.Button(Editor.GetUIWord(UIWORD.ResetPivot), apGUILOFactory.I.Height(30)))
			{
				//아예 함수로 만들것
				//이전 코드
				//>> OffsetPos만 바꾸는 코드
			...
			//v1.6.2 : 버텍스들의 중심으로 Pivot 설정
			if (GUILayout.Button(Editor.GetTextV3(TEXTV3.SetCenterAsPivot), apGUILOFactory.I.Height(30)))
			{
				int nVerts = mesh._vertexData != null ? mesh._vertexData.Count : 0;

				//AABB의 중심을 Pivot으로 삼자
				if(nVerts > 0)
				{
					...
					{
						vert = mesh._vertexData[i];
						
						if(vert == null) { continue; }

						if(i == 0)
						{
							minPos.x = vert._pos.x;
							minPos.y = vert._pos.y;
							maxPos.x = vert._pos.x;
							maxPos.y = vert._pos.y;
						}
						else
						{
							minPos.x = Mathf.Min(minPos.x, vert._pos.x);
							minPos.y = Mathf.Min(minPos.y, vert._pos.y);
							maxPos.x = Mathf.Max(maxPos.x, vert._pos.x);
							maxPos.y = Mathf.Max(maxPos.y, vert._pos.y);
						}
					}

					centerPos = (minPos + maxPos) * 0.5f;

					Editor.Controller.SetMeshPivot(Editor.Select.Mesh, centerPos);
				}
				else
				{
					EditorUtility.DisplayDialog(	Editor.GetTextV3(TEXTV3.DLG_PivotToCenterFailed_Title),
													Editor.GetTextV3(TEXTV3.DLG_PivotToCenterFailed_Body),
													Editor.GetText(TEXT.Okay));
				}
			}
		}
