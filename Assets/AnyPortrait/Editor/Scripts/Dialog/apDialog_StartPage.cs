...
namespace AnyPortrait
{
	...
	public class apDialog_StartPage : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			//버튼 두개씩
			//단, 중국버전에서는 "동영상 튜토리얼"이 제외된다.
			int width_GettingStarted = width_Half;
			bool isCNVersion = false;
			if (apVersion.I.IsCNStore)
			{
				width_GettingStarted = (width - 10) + 1;
				isCNVersion = true;
			}
			...
			if(GUILayout.Button(_guiContent_GettingStarted, GUILayout.Width(width_GettingStarted), GUILayout.Height(height_4Btn)))
			{
			...
			if(!isCNVersion)
			{
				if(GUILayout.Button(_guiContent_VideoTutorial, GUILayout.Width(width_Half), GUILayout.Height(height_4Btn)))
				{
					...
					if(_editor._language == apEditor.LANGUAGE.Korean)//한국어
					{
						...
						Application.OpenURL("https://rainyrizzle.com/ko/ap-video-tutorials-ko/");//v1.6.1
					}
					else if(_editor._language == apEditor.LANGUAGE.Japanese)//일본어
					{
						//Application.OpenURL("https://www.rainyrizzle.com/ap-videotutorial-jp");
						Application.OpenURL("https://rainyrizzle.com/ja/ap-video-tutorials-ja/");//v1.6.1
					}
					else//영어 + 기타
					{
						//Application.OpenURL("https://www.rainyrizzle.com/ap-videotutorial-eng");
						Application.OpenURL("https://rainyrizzle.com/en/ap-video-tutorials-en/");//v1.6.1
					}
				}
			}
			...
			if(GUILayout.Button(_guiContent_Forum, GUILayout.Width(width_Half), GUILayout.Height(height_4Btn)))
			{
				...
				if(_editor._language == apEditor.LANGUAGE.Korean)//한국어
				{
					Application.OpenURL("https://rainyrizzle.com/ko/forum-ko/");
				}
				else if(_editor._language == apEditor.LANGUAGE.Japanese)//일본어
				{
					Application.OpenURL("https://rainyrizzle.com/ja/forum-ja/");
				}
				else//영어 + 기타
				{
					Application.OpenURL("https://rainyrizzle.com/en/forum-en/");
				}
			}
			EditorGUILayout.EndHorizontal();
			...
			else
			{
				...
				if (GUILayout.Button(_editor.GetText(TEXT.DLG_CheckLimitations), GUILayout.Height(25)))//"Check Limitations"
				{
					...
					if(_editor._language == apEditor.LANGUAGE.Korean)//한국어
					{
						Application.OpenURL("https://rainyrizzle.com/ko/free-demo-download-ko/");
					}
					else if(_editor._language == apEditor.LANGUAGE.Japanese)//일본어
					{
						Application.OpenURL("https://rainyrizzle.com/ja/free-demo-download-ja/");
					}
					else//영어 + 기타
					{
						Application.OpenURL("https://rainyrizzle.com/en/free-demo-download-en/");
					}
