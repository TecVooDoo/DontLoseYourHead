	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(vertexOutput IN) : COLOR
			{
				...
				//maskResult *= GetMaskAlpha(saturate(maskTexture.r * _MaskColor.a), saturate(_MaskRatio));//이전
				maskResult *= GetMaskAlpha(maskTexture.r, saturate(_MaskRatio));
